 ...
 * ==========================================
 * 📌 代码名称: 📅 岁时黄历（极客深紫 + 进度条版）
 * ✨ 特色功能: 保留原版庞大本地算法(防断网)，替换 Crypto 配色，底部进度条，新增本地冲煞与百忌推算。
 * ==========================================
 */
...
export default async function(ctx) {
  ...
  const THEME_ACCENT_GOLD = { light: '#FFD700', dark: '#FFD700' }; 
  const THEME_ACCENT_GREEN = { light: '#32D74B', dark: '#32D74B' };
  const PROGRESS_COLOR = { light: '#9945FF', dark: '#9945FF' };
  ...
  const percent = ((now - startOfYear) / (endOfYear - startOfYear) * 100).toFixed(1);
  const filled = Math.floor(percent / 10);
  const bar = '█'.repeat(filled) + '░'.repeat(10 - filled);
  const progStr = `今年进度 ${bar} ${percent}%`;
  ...
  return {
    ...
    children: [
      ...
      { type: 'stack', direction: 'row', alignItems: 'center', gap: 6, children: [
          { type: 'image', src: 'sf-symbol:hourglass', color: PROGRESS_COLOR, width: 13, height: 13 },
          { type: 'text', text: progStr, font: { size: 12, weight: 'medium', family: 'Menlo' }, textColor: TEXT_SUB, maxLines: 1 }
      ]},
      { type: 'spacer' }
    ]
  };
